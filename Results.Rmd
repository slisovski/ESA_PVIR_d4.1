...
<details>
  ...
  pl2 <- map +
    geom_raster(data=rast_df, aes(x=x, y=y, fill=value), alpha=0.8, show.legend = FALSE) + 
      ...
      labs(x = "", y = "", col = "", size = "Nr. measurements") +
...
```{r, include = FALSE, eval = FALSE}
  ...
  geom_ribbon(data = dat[[1]], mapping = aes(x = Depth, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[1]], mapping = aes(x = Depth, y = plwr), col = "grey10", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[1]], mapping = aes(x = Depth, y = pupr), col = "grey10", size = 0.5, linetype = "dashed") +
  geom_ribbon(data = dat[[2]] %>% filter(Depth<max(resTab %>% filter(!coldSite) %>% pull(Depth))), mapping = aes(x = Depth, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[2]] %>% filter(Depth<max(resTab %>% filter(!coldSite) %>% pull(Depth))), 
              mapping = aes(x = Depth, y = plwr), col = "firebrick", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[2]] %>% filter(Depth<max(resTab %>% filter(!coldSite) %>% pull(Depth))), 
              mapping = aes(x = Depth, y = pupr), col = "firebrick", size = 0.5, linetype = "dashed") +
  geom_ribbon(data = dat[[3]], mapping = aes(x = Depth, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[3]], mapping = aes(x = Depth, y = plwr), col = "cornflowerblue", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[3]], mapping = aes(x = Depth, y = pupr), col = "cornflowerblue", size = 0.5, linetype = "dashed") +
  
  geom_point(data = resTab, mapping = aes(Depth, res, col = coldSite), alpha = 0.1, shape = 16, size = 1.5) +
  
  geom_ribbon(data = dat[[1]], mapping = aes(x = Depth, y = fit, ymin = lwr, ymax = upr), alpha = 0.15, fill = "grey10") +
    geom_line(data = dat[[1]], mapping = aes(x = Depth, y = fit), col = "grey10", size = 1.5) +
  geom_ribbon(data = dat[[2]] %>% filter(Depth<max(resTab %>% filter(!coldSite) %>% pull(Depth))), 
              mapping = aes(x = Depth, y = fit, ymin = lwr, ymax = upr), alpha = 0.15, fill = "grey10") +
    geom_line(data = dat[[2]] %>% filter(Depth<max(resTab %>% filter(!coldSite) %>% pull(Depth))), 
              mapping = aes(x = Depth, y = fit), col = "firebrick", size = 1.5) +
  geom_ribbon(data = dat[[3]], mapping = aes(x = Depth, y = fit, ymin = lwr, ymax = upr), alpha = 0.15, fill = "grey10") +
    geom_line(data = dat[[3]], mapping = aes(x = Depth, y = fit), col = "cornflowerblue", size = 1.5) +
  
  labs(y = "Residuals", x = "MAGT [˚C] in situ data") +
  theme(text = element_text(size=rel(4.5)))



#### over Latitudes
model1   <- gam(res ~ s(Latitude), data = resTab)
model2   <- gam(res ~ s(Latitude), data = resTab %>% filter(!coldSite))
model3   <- gam(res ~ s(Latitude), data = resTab %>% filter(coldSite))
...
ggplot(NULL, aes(x, y)) +
  theme_bw() +
  geom_abline(slope = 0, intercept = 0, linetype = "dashed") +
  geom_ribbon(data = dat[[1]], mapping = aes(x = Latitude, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[1]], mapping = aes(x = Latitude, y = plwr), col = "grey10", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[1]], mapping = aes(x = Latitude, y = pupr), col = "grey10", size = 0.5, linetype = "dashed") +
  geom_ribbon(data = dat[[2]], mapping = aes(x = Latitude, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[2]], mapping = aes(x = Latitude, y = plwr), col = "firebrick", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[2]], mapping = aes(x = Latitude, y = pupr), col = "firebrick", size = 0.5, linetype = "dashed") +
  geom_ribbon(data = dat[[3]], mapping = aes(x = Latitude, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[3]], mapping = aes(x = Latitude, y = plwr), col = "cornflowerblue", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[3]], mapping = aes(x = Latitude, y = pupr), col = "cornflowerblue", size = 0.5, linetype = "dashed") +
  
  geom_point(data = resTab, mapping = aes(Latitude, res, col = coldSite), alpha = 0.1, shape = 16, size = 1.5) +
  ...
  geom_ribbon(data = dat[[2]], mapping = aes(x = Latitude, y = fit, ymin = lwr, ymax = upr), alpha = 0.15, fill = "grey10") +
    geom_line(data = dat[[2]], mapping = aes(x = Latitude, y = fit), col = "firebrick", size = 1.5) +
  geom_ribbon(data = dat[[3]], mapping = aes(x = Latitude, y = fit, ymin = lwr, ymax = upr), alpha = 0.15, fill = "grey10") +
    geom_line(data = dat[[3]], mapping = aes(x = Latitude, y = fit), col = "cornflowerblue", size = 1.5) +
...
#### over Longitudes
model1   <- gam(res ~ s(Longitude), data = resTab)
model2   <- gam(res ~ s(Longitude), data = resTab %>% filter(!coldSite))
model3   <- gam(res ~ s(Longitude), data = resTab %>% filter(coldSite))

### confidence interval
newdat     <- data.frame(Longitude = seq(min(resTab$Longitude), max(resTab$Longitude), length = 100))
dat        <- lapply(list(model1, model2, model3), predInterval, newdat = newdat)

ggplot(NULL, aes(x, y)) +
  theme_bw() +
  geom_abline(slope = 0, intercept = 0, linetype = "dashed") +
  geom_ribbon(data = dat[[1]], mapping = aes(x = Longitude, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[1]], mapping = aes(x = Longitude, y = plwr), col = "grey10", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[1]], mapping = aes(x = Longitude, y = pupr), col = "grey10", size = 0.5, linetype = "dashed") +
  geom_ribbon(data = dat[[2]], mapping = aes(x = Longitude, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[2]], mapping = aes(x = Longitude, y = plwr), col = "firebrick", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[2]], mapping = aes(x = Longitude, y = pupr), col = "firebrick", size = 0.5, linetype = "dashed") +
  geom_ribbon(data = dat[[3]], mapping = aes(x = Longitude, y = fit, ymin = plwr, ymax = pupr), alpha = 0.15) +
    geom_line(data = dat[[3]], mapping = aes(x = Longitude, y = plwr), col = "cornflowerblue", size = 0.5, linetype = "dashed") +
    geom_line(data = dat[[3]], mapping = aes(x = Longitude, y = pupr), col = "cornflowerblue", size = 0.5, linetype = "dashed") +
  
  geom_point(data = resTab, mapping = aes(Longitude, res, col = coldSite), alpha = 0.1, shape = 16, size = 1.5) +
  ...
  geom_ribbon(data = dat[[2]], mapping = aes(x = Longitude, y = fit, ymin = lwr, ymax = upr), alpha = 0.15, fill = "grey10") +
    geom_line(data = dat[[2]], mapping = aes(x = Longitude, y = fit), col = "firebrick", size = 1.5) +
  geom_ribbon(data = dat[[3]], mapping = aes(x = Longitude, y = fit, ymin = lwr, ymax = upr), alpha = 0.15, fill = "grey10") +
    geom_line(data = dat[[3]], mapping = aes(x = Longitude, y = fit), col = "cornflowerblue", size = 1.5) +
  
  labs(y = "Residuals", x = "Longitude of MAGT [˚C] in situ data") +
  theme(text = element_text(size=rel(4.5)))

png("figures/resTmp.png", width = 30, height = 50, units = "cm", res = 250)
gridExtra::grid.arrange(pl1, pl2, pl3, pl4, pl5, nrow = 5)
dev.off()
